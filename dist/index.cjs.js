...
class TsVisitor extends visitorPluginCommon.BaseTypesVisitor {
    ...
    NamedType(node, key, parent, path, ancestors) {
        const astNode = this._schema.getType(node.name).astNode;
        let wrapType = null;
        if (astNode && astNode.directives) {
            const wrap = astNode.directives
                .find(d => d.name.value == "wrapWith");
            if (wrap
                && 'value' in wrap.arguments[0].value
                && typeof wrap.arguments[0].value.value === 'string')
    ...
    InputValueDefinition(node, key, parent) {
        ...
        const addOptionalSign = !this.config.avoidOptionals.inputValue &&
            (originalFieldNode.type.kind !== graphql.Kind.NON_NULL_TYPE || node.defaultValue !== undefined);
